...
# Drawing functions
# Draw a random line and output the positions of the endpoints
def draw_lines(img):
    ...
    for i in range(num_lines):
        ...
        for j in range(len(segments)):
            if intersect((x1, y1), (x2, y2), segments[j][0], segments[j][1]):
                flag = True
                break
        if flag:
            continue
...
def draw_multiple_polygons(img):
    ...
    rads = []
    points = np.array([[]])
    for i in range(20):  # at most 10 polygons (if no overlap)
        num_corners = np.random.randint(3, 6)
        rad = max(np.random.rand() * min(img.shape[0] / 2, img.shape[1] / 2), 30)
        x = np.random.randint(rad, img.shape[1] - rad)  # Select the center of a circle
        y = np.random.randint(rad, img.shape[0] - rad)
        ...
        angles = [slices[i] + np.random.rand() * (slices[i+1] - slices[i])
                  for i in range(num_corners)]
        new_points = np.array([[int(x + max(np.random.rand(), 0.5) * rad * math.cos(a)),
                                int(y + max(np.random.rand(), 0.5) * rad * math.sin(a))]
                               for a in angles])
...
def draw_checker_board(img):
    ...
    affine_transform = np.array([[max(np.random.rand(), 0.7),
                                 ...
                                 [min(np.random.rand(), 0.3),
                                  ...
                                  np.random.randint(50)]])
    warped_board = np.transpose(cv.warpAffine(board, affine_transform, img.shape[0:2]))
...
def draw_cube(img):
    ...
    translation = np.array([img.shape[1] / 2 + np.random.randint(-img.shape[1] / 5,
                                                                 img.shape[1] / 5),
                            img.shape[0] / 2 + np.random.randint(-img.shape[0] / 5,
                                                                 img.shape[0] / 5),
                            0])
    cube = translation + np.transpose(np.dot(scaling,
                                             np.dot(rotation_1,
                                                    np.dot(rotation_2,
                                                           np.dot(rotation_3,
                                                                  np.transpose(cube))))))
    ...
    fbitset = bitset.Bitset(bpoint, 3)
    for i in [0, 1, 2]:
    ...
    for i in [0, 1, 2]:
        current_bit = bitset.Bitset(fpoint, 3)
        idx = 1
        for j in [0, 1, 0]:
    ...
    col_face = get_random_color()
    for i in [0, 1, 2]:
        cv.fillPoly(img, [cube[faces[i]].reshape((-1, 1, 2))],
                    (col_face, col_face, col_face))
    col_edge = (col_face + 128) % 256  # get a color that constrats with the face color
    thickness = np.random.randint(1, 3)
    for i in [0, 1, 2]:
        for j in [0, 1, 2, 3]:
...
def draw_shape(img):
    ...
    if idx == 2:
        return draw_ellipse(img)
    if idx == 3:
        return draw_star(img)
    if idx == 4:
        return draw_checker_board(img)
    else:
...
# Main function
# Create a window with a random shape in it
window_name = "Output"

# Create grey empty images
size = (H, W, 3)
# img = np.zeros(size, dtype=np.uint8) + get_random_color()
img = generate_background(size)
# img = add_gaussian_noise(img, 128, 50) # add Gaussian noise
# img = add_salt_and_pepper(img) # add salt-and-pepper noise

# points = draw_lines(img)
# points = draw_polygon(img)
# points = draw_multiple_polygons(img)
# points = draw_ellipse(img)
# points = draw_star(img)
# points = draw_checker_board(img)
# points = draw_cube(img)
points = draw_shape(img)
draw_interest_points(img, points)
